...
Controllo degli accessi: far accedere solo la persona autorizzata. Uno degli obbiettivi principali dell'OS è quello di introdurre dei meccanismi che garntiscono confidenzialità, intregrità, disponibilità delle informazioni che vengono inseriti del sistema.

Il sistema di protezione è costituito da 3 entità principali:
- Il meccanismo di authenticazione: il sistema verifica l'identità di chi sta usando, 
...
B. Lampson **regole auree** perchè tutte le regole iniziano con Au
...
- **Operazoni di accesso**: _diritti di acesso_ cosa puo fare l'utente.
...
- Prevenire accesso non aurizzato.
...
ACL viene usata per fare security checkings. Un utente chiede di accedere ad una risorsa e si va a controllare la lista per vedere s'è autorizzata o meno. solitamente l'OS conosce il processo e quindi l'OS deve accedere al i-node e recuperare ACL.
...
Componenete specifica dell'OS che fa access control è il **Reference Monitor** è un insieme di componenti sono distribuite in diversi parti: nel kernel, file system, drivers, ecc. mirate a garantire la sicurezza dell'intero sistema.
...
I processi unix sono contradistnti da un pid, è un numero che li identifica all'interno del sistema. Un nuovo processo viene generato attraverso il syscall fork().

Un processo a sua volta ha un `gid` e un `uid` assocciato:

- Real uid e gid `RUID`/`rgid` identifica il sogetto che l'ha generato e viene **ereditato dal processo figlio dal processo genitore**, generalmente uid e gid dell'utente che è loggato.
...
Per **ogni oggetto in UNIX esistono solo tre utenti**. Owner, grouppo primario dell'owner, resto del mondo. `user | group | other `.
...
`execve` **protrebbe modificare** `EUID` del processo: quando si fa fork si eredita RUID ed EUID del genitore e quando il processo figlio fa execve è possbibile che modifichi l'EUID (prende quello del file). Es. utente 1000 manda in esecuzione il commando `ls`, `ls` ha come `uid 0` perchè è di `root`, quindi il processo ls che l'utente ha eseguito avrà `RUID 1000` ed `EUID 0`. Questo succede solo ai file `setuid`.
...
L'OS utilizza delle **maschere** per assegnare i diritti ACL. Se un utente crea un file il sistema assegnerà un set di diritti predifito, eventualmente l'utente potrà modificare secondo le sue esigenze.
